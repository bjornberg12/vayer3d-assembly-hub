...
// 10m mast. Main vertical mast at origin. A secondary angled brace pole
// (as in the photo: an A-frame style support running up to the crossarm).
...
function Insulator({ position }: { position: [number, number, number] }) {
  // Stack of red ceramic discs on a small metal pin
  return (
...
export function ElectricalPost() {
  // Main mast at origin, runs straight up to 10m
  // Brace pole leans in from -1.6m on X, meeting the crossarm at top
  ...
  // Crossarm sits horizontally near top, spanning along Z
  ...
  // 3 insulator/phase positions along the crossarm
  ...
  // Where the actual conductor sits (top of insulator)
  ...
  return (
    <group>
      ...
      {/* Brace mast (slightly darker, as in photo) */}
      ...
      {/* Metal banding where crossarm meets mast */}
      ...
      {phasePositions.map((p, i) => (
        <Insulator key={i} position={p} />
      ))}

      {/* Phase lines (3 conductors) running into the distance on both sides along X */}
      {phasePositions.map((p, i) => {
